...
for h in sds.k2_gather_subobject_data_for_blueprint(bp):
    ...
    if o and "Switcher" in o.get_class().get_name():
        o.set_editor_property("StateFadeMaterials", rest)       # 不透明SSS（静止+显隐都用它）
        o.set_editor_property("StateDissolveMaterials", empty)  # 清空→不做溶解，靠粒子过渡
        o.set_editor_property("SourceHideDelay", 0.25)          # 粒子起来后尽快隐藏旧模型
        o.set_editor_property("SourceFadeOutDuration", 0.3)
        out["bp"]=True; break
...
open("C:/UE_Project/MCPGameProject/Scripts/_opt2.json","w").write(json.dumps(out,ensure_ascii=False))
