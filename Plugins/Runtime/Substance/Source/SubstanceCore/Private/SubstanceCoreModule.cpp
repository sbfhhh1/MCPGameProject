...
#				define SUBSTANCE_LIB_GPU_DYNAMIC_PATH	TEXT("DLLs/" SUBSTANCE_LIB_CONFIG "/Win64/substance_d3d11_blend.dll")
#		else
#			error Unsupported platform for dynamic substance loading
#		endif
#   elif PLATFORM_MAC
#       define SUBSTANCE_LIB_CPU_DYNAMIC_PATH TEXT("DLLs/" SUBSTANCE_LIB_CONFIG "/Mac/libsubstance_cpu_blend.dylib")
...
#		define SUBSTANCE_LIB_GPU_DYNAMIC_PATH TEXT("DLLs/" SUBSTANCE_LIB_CONFIG "/Linux/libsubstance_vk_blend.so")
#   else
#       error Unsupported platform for dynamic substance loading
#	endif
#endif
...
void FSubstanceCoreModule::StartupModule()
{
	...
	{
        FString ContentDir = IPluginManager::Get().FindPlugin(TEXT("Substance"))->GetBaseDir();
		FString libraryPath = FPaths::Combine(ContentDir, libraryFileName);
		if (FPaths::FileExists(libraryPath))
		{
	...
	if (!IsRunningDedicatedServer())
	{
		...
		TickDelegate = FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateRaw(this, &FSubstanceCoreModule::Tick));
