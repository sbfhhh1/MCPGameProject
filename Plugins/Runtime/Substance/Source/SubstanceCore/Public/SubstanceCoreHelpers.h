...
//! @brief Acquire the most recent render result from an output
//! @param output Substance framework output object
//! @return Pointer to the render result if available, nullptr if empty
//! @note If there are older results, they will be cleared and freed
...
SUBSTANCECORE_API bool IsInputVisible(const SubstanceAir::InputInstanceBase& Input);
...
/** Exports the selected preset values */
SUBSTANCECORE_API bool ExportSelectedPresets(USubstanceGraphInstance* graph, const TArray<FString>& selectedPresets);

/** Import Preset File */
SUBSTANCECORE_API TArray<FString> ImportPresets(USubstanceGraphInstance* graph);
...
/** Determine preferable RGB format of output instance channel use */
SUBSTANCECORE_API unsigned int GetDefaultChannelFormat(SubstanceAir::OutputInstance* Instance);
