...
struct FSubstanceInputDesc
{
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Values")
	TEnumAsByte<ESubstanceInputType> Type = SIT_Float;
};
...
public:
	...
	/** Array of presets that the user can switch between */
	UPROPERTY()
	TArray<FSubstancePreset> Presets;

	/** Keeps track of the current preset */
	FText CurrentPreset;
	...
	*/
	...
	/** Input delta values used for serializtion of inputs */
	TSharedPtr<SubstanceAir::Preset> InstancePreset;

	FString GetCurrentPresetAsString() const;

	/** Creates our data representation for embedded presets */
	void InitEmbeddedPresets();

	/** Add Substance Air presets to container */
	void AddPresets(const SubstanceAir::Presets& presets);

	/** Update Substance Air preset */
	void PresetUpdate(const FString& presetLabel);

	/** Preset Selected to apply */
	void ApplyPreset(const FString& presetLabel);

	/** Preset to clear from the list */
	void RemovePreset(const FString& presetLabel);

	/** Create a new preset using the current preset values */
	void CreatePreset(const FString& presetName);

	/** Returns an array of all of the active presets */
	TArray<FString> GetAllPresetLabels();

	/** Invalidate current preset due to inputs having been modified */
	void InvalidateCurrentPreset();
