...
FRotator FUnrealMCPCommonUtils::GetRotatorFromJson(const TSharedPtr<FJsonObject>& JsonObject, const FString& FieldName)
{
    ...
    // 支持对象格式 {pitch, yaw, roll} (来自 MCP 桥接)
    const TSharedPtr<FJsonObject>* JsonObj;
    if (JsonObject->TryGetObjectField(FieldName, JsonObj))
    {
                                     ...
                                     const TSharedPtr<FJsonValue>& Value, FString& OutErrorMessage)
{
    ...
    else if (Property->IsA<FStructProperty>())
    {
        ...
        {
            FLinearColor* ColorPtr = StructProp->ContainerPtrToValuePtr<FLinearColor>(Object);
            const TSharedPtr<FJsonObject>* ObjValue;
            if (Value->TryGetObject(ObjValue))
            {
                (*ObjValue)->TryGetNumberField(TEXT("R"), ColorPtr->R);
                (*ObjValue)->TryGetNumberField(TEXT("G"), ColorPtr->G);
                (*ObjValue)->TryGetNumberField(TEXT("B"), ColorPtr->B);
                (*ObjValue)->TryGetNumberField(TEXT("A"), ColorPtr->A);
                return true;
            }
        }
        if (StructProp && StructProp->Struct == TBaseStructure<FVector>::Get())
        {
            FVector* VecPtr = StructProp->ContainerPtrToValuePtr<FVector>(Object);
            const TSharedPtr<FJsonObject>* ObjValue;
            if (Value->TryGetObject(ObjValue))
            {
                (*ObjValue)->TryGetNumberField(TEXT("X"), VecPtr->X);
                (*ObjValue)->TryGetNumberField(TEXT("Y"), VecPtr->Y);
                (*ObjValue)->TryGetNumberField(TEXT("Z"), VecPtr->Z);
                return true;
            }
        }
    }
