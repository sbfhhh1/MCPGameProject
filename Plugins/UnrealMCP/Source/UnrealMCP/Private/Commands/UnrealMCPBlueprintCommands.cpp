...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleCreateBlueprint(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (!ParentClass.IsEmpty())
    {
        ...
        else
        {
            ...
            if (!FoundClass)
            {
                // Try the project module path
                const FString ProjectClassPath = FString::Printf(TEXT("/Script/MCPGameProject.%s"), *ClassName);
                FoundClass = LoadClass<AActor>(nullptr, *ProjectClassPath);
            }
            if (!FoundClass)
            {
            ...
            if (!FoundClass)
            {
                // Generic search: iterate all UClass objects
                for (TObjectIterator<UClass> ClassIt; ClassIt; ++ClassIt)
                {
                    if (ClassIt->IsChildOf<AActor>() && !ClassIt->HasAnyClassFlags(CLASS_Abstract))
                    {
                        FString ClassNameNoA = ClassIt->GetName();
                        if (ClassNameNoA == ClassName || ClassNameNoA == ParentClass)
                        {
                            FoundClass = *ClassIt;
                            break;
                        }
    ...
    if (NewBlueprint)
    {
        ...
        // Save the package to disk so spawn_blueprint_actor can find it
        UEditorAssetLibrary::SaveAsset(PackagePath + AssetName, true);
